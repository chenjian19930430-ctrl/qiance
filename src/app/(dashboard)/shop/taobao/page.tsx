...
interface TaobaoShop extends Record<string, unknown> {
  id: string
  name: string
  code: string
  platform: string
...
const columns: ColumnDef<TaobaoShop>[] = [
...
export default function TaobaoShopPage() {
  return (
    <CrudPage<TaobaoShop>
      title="淘宝店铺管理"
      description="管理淘宝平台店铺信息"
      columns={columns}
      fields={fields}
      fetchData={async (params) => api.get("/api/shop?platform=taobao", params)}
      onCreate={async (values) => api.post("/api/shop", { ...values, platform: "taobao", channel: "taobao" })}
      onUpdate={async (id, values) => api.put(`/api/shop?id=${id}`, values)}
      onDelete={async (id) => api.delete(`/api/shop?id=${id}`)}
      getId={(row) => row.id}
      searchable
      searchPlaceholder="搜索淘宝店铺名称..."
    />
  )
}
