...
export async function GET(req: Request) {
  try {
    ...
    const [list, total] = await Promise.all([
      prisma.post.findMany({ where, skip: (page - 1) * pageSize, take: pageSize, orderBy: { sort: "asc" } }),
      prisma.post.count({ where }),
    ])

    return NextResponse.json({ code: 200, data: { list, total, page, pageSize }, message: "success" })
  } catch (error) {
...
export async function PUT(req: Request) {
  try {
    ...
    const data = await prisma.post.update({ where: { id }, data: body })
...
export async function POST(req: Request) {
  try {
    ...
    const data = await prisma.post.create({
      data: { name: body.name, code: body.code, sort: body.sort || 0, status: body.status ?? 0, remark: body.remark, tenantId: body.tenantId || "default" },
    })
