...
/** 检查 API Key 是否真实可用（跳过占位符 sk-xxx 和空值） */
function isRealKey(key: string | undefined): boolean {
  if (!key) return false
  // 检测占位符: sk-xxx, sk-your-key, 留空, 或其他非真实格式
...
export async function createCompletion(request: CompletionRequest): Promise<CompletionResponse> {
  ...
  try {
    ...
    if (provider === "minimax") {
      return await callMiniMax(fullMessages, modelName, temperature, maxTokens, agent, systemPrompt, agentCode)
    }

    return await callOpenAI(fullMessages, modelName, temperature, maxTokens, agent, systemPrompt, agentCode)
  } catch (error) {
